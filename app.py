...
def load_data():
    ...
    # Thêm đọc sheet Wood
    url_wood = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/gviz/tq?tqx=out:csv&sheet=Wood"
    df_wood = pd.read_csv(url_wood)
...
wood_base_rate = (df_wood['Gem2trade'] / df_wood['Qnt']).min() * gem_base_rate # VND / 1 Gỗ
...
with col2:
    ...
    goal_options = ["Tối đa số lượng", "ROI lớn nhất (Giá trị VND thực tế lớn nhất)"]
    ...
    if st.button("Tối ưu Quy Đổi", type="primary"):
        ...
        if usable_gems < 0:
            ...
            elif target_2 == "Gỗ (Wood)":
                df_target = df_wood.copy()
                if goal_2 == "Tối đa số lượng":
                    df_target['Optimize_Value'] = df_target['Qnt']
                else: 
                    df_target['Optimize_Value'] = df_target['Qnt'] * wood_base_rate
                res, max_val, rem = optimize_knapsack(usable_gems, df_target, 'Woodpack', 'Gem2trade', 'Optimize_Value')
                total_wood = sum([v * df_target[df_target['Woodpack']==k]['Qnt'].values[0] for k, v in res.items()])
                total_gold = 0
                total_card = 0
            ...
            if target_2 == "Vàng (Gold)":
                ...
                st.write(f"- **Gỗ thu được:** {int(total_wood):,} 🪵")
            else: # Combo
                st.write(f"- **Vàng thu được:** {int(total_gold):,} 🪙")
...
if st.button("ROI OF SEASONAL PACKAGE", type="primary"):
    try:
        ...
        if total_pack_value_vnd > 0:
            ...
            ratio_wood = val_from_wood / total_pack_value_vnd
            ratio_card = val_from_card / total_pack_value_vnd
        else:
            ratio_gem = ratio_gold = ratio_wood = ratio_card = 0
        ...
        std_wood_bought = (cost_in_vnd * ratio_wood) / wood_base_rate if wood_base_rate > 0 else 0
        ...
        if pack_wood > 0:
            st.write(f"- 🪵 **{std_wood_bought:,.0f} Gỗ** *(Mức m đang được nhận: {pack_wood:,.0f})*")
        if pack_card > 0:
